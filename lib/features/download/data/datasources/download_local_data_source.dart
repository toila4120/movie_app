...
class DownloadLocalDataSourceImpl implements DownloadLocalDataSource {
  ...
  Future<void> _onUpgrade(Database db, int oldVersion, int newVersion) async {
    if (oldVersion < 2) {
      // Thêm các cột mới cho version 2
      await db.execute('''
        ...
        ADD COLUMN url TEXT;
        ALTER TABLE $tableName 
        ADD COLUMN is_downloading INTEGER DEFAULT 0;
      ''');
  ...
  Future<void> _createIndexesInternal(Database db) async {
    ...
    // Index cho episode_id
    await db.execute('''
      CREATE INDEX IF NOT EXISTS idx_episode_id 
      ON $tableName (episode_id)
    ''');
  }
  ...
  Future<DownloadEpisode?> getEpisode(String episodeId) async {
    ...
    return DownloadEpisode(
      episodeId: maps[0]['episode_id'] as String,
      title: maps[0]['title'] as String,
      url: maps[0]['url'] as String,
      isDownloading: maps[0]['is_downloading'] == 1,
      progress: maps[0]['progress'] as double,
      localPath: maps[0]['localPath'] as String?,
    );
  }
  ...
  Future<bool> isDownloading(String episodeId) async {
    ...
    final List<Map<String, dynamic>> maps = await db.query(
      tableName,
      columns: ['is_downloading'],
      where: 'episode_id = ?',
      whereArgs: [episodeId],
    );

    if (maps.isEmpty) return false;
    return maps[0]['is_downloading'] == 1;
  }
